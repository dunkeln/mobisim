...
function normalizeAudioError(
  error: unknown
): OpenAIChatInputError | OpenAIChatConfigError | OpenAIChatExecutionError | OpenAIChatUpstreamError {
  ...
  if (
    ...
    error instanceof OpenAIChatExecutionError ||
  ...
  return new OpenAIChatExecutionError(
    error instanceof Error ? error.message : 'Audio chat request failed.'
  );
}
...
async function persistContextHistorySafely(input: {
  ...
  try {
    ...
    console.error(
      'chat audio history persistence failed',
